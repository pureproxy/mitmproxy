...
export class CertManagerFs extends EventEmitter {
  ...
  getCaKeysAndCert(hostname, port) {
    ...
    caItem = generateCaKeysAndCert({
      commonName: this.defaultCaCommonName,
      keyLength: this.caKeyLength,
    })
  ...
  getServerKeysAndCert(hostname, port) {
    ...
    if (
      serverItem &&
      serverItem.cert.validity.notAfter.getTime() > Date.now()
    ) {
      return serverItem
    }
    ...
    serverItem = generateServerKeysAndCert({
      commonName: hostname,
      caKeysAndCert,
      keyLength: this.serverKeyLength,
    })
