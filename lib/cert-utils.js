...
export const generateDateOffsetByYears = (years) => {
  const date = new Date()
...
/**
 * @param {{serialNumber?: number, keyLength?: number, notBefore?: Date, notAfter?: Date}} [options]
 */
export const generateGenericKeysAndCert = (options) => {
  const {
    serialNumber = generateSerialNumber(),
    keyLength = 2048,
    notBefore = generateDateOffsetByYears(-10),
    notAfter = generateDateOffsetByYears(10),
  } = options || {}
...
/**
 * @param {{commonName: string, serialNumber?: number, keyLength?: number, notBefore?: Date, notAfter?: Date}} options
 */
export const generateCaKeysAndCert = (options) => {
  const { commonName, serialNumber, keyLength, notBefore, notAfter } = options

  if (!commonName) {
    throw new Error(`Undefined commonName`)
  }

  const { keys, cert } = generateGenericKeysAndCert({
    serialNumber,
    keyLength,
    notBefore,
    notAfter,
...
/**
 * @param {{commonName: string, caKeysAndCert: Record<string, any>, serialNumber?: number, keyLength?: number, notBefore?: Date, notAfter?: Date}} options
 */
export const generateServerKeysAndCert = (options) => {
  let {
    commonName,
    caKeysAndCert,
    serialNumber,
